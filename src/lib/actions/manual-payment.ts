...
import { makeExternalId } from "@/lib/kpay";
...
export async function declareManualSubscriptionPaymentAction(input: {
  ...
  const senderPhone = normalizeCameroonPhone(input.senderPhone);
  if (senderPhone.length !== 12 || !senderPhone.startsWith("237")) {
    return { ok: false, error: "Numéro Cameroun invalide" };
  }

  // Standard = 1 semaine fixe, sans multiplicateur ni remise. Premium/VIP = mensuel 1/3/12.
  const months = input.tier === "STANDARD"
    ? (1 as const)
    : (([1, 3, 12].includes(input.months) ? input.months : 1) as 1 | 3 | 12);
  ...
  // Pas de remise sur le Standard (période fixe unique).
  const discountPercent = input.tier === "STANDARD" ? 0 : months >= 12 ? 15 : months >= 3 ? 5 : 0;
  const amount = Math.round(pricing.amount * months * (1 - discountPercent / 100));
  const days = pricing.days * months;
...
/**
 * Déclaration manuelle du paiement de vérification d'identité (Mobile Money
 * direct, hors K-Pay). Crée le Payment PENDING + une IdVerification PENDING
 * (sans documents — envoyés ensuite directement sur Telegram, cf.
 * /escort/verification). Un admin valide le paiement côté /admin/paiements
 * puis marque le profil vérifié depuis la fiche utilisateur une fois les
 * documents examinés sur Telegram.
 */
export async function declareManualVerificationPaymentAction(input: {
  senderPhone: string;
  reference: string;
}): Promise<DeclareManualPaymentResult> {
  const session = await auth();
  ...
  const ip = (await headers()).get("x-forwarded-for") ?? "anon";
  const limited = await rateLimit(`manualverif:${session.user.id}:${ip}`, {
    limit: 5,
    windowMs: 60_000,
  });
  if (!limited.success) return { ok: false, error: "Trop de tentatives, réessayez dans une minute" };
  ...
  // Anti-spam : une seule vérification en attente à la fois
  const pending = await prisma.idVerification.findFirst({
    where: { userId: session.user.id, status: "PENDING" },
    select: { id: true },
  });
  if (pending) {
    return { ok: false, error: "Une vérification est déjà en cours." };
  }
  ...
  const [payment] = await prisma.$transaction([
    prisma.payment.create({
      data: {
        ...
        provider: "MANUAL",
        status: "PENDING",
        providerRef: makeExternalId("manualverif", session.user.id),
        intent: intent as unknown as Prisma.InputJsonValue,
        metadata: { declaredPhone: senderPhone, declaredReference: reference, submittedVia: "manual" },
      },
    }),
